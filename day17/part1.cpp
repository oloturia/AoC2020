...
int elab(string filename) {
	...
	string temp;
	map<tuple<int,int,int>,bool> space;
	map<tuple<int,int,int>,bool> to_check;
	map<tuple<int,int,int>,bool> next_matrix;
	tuple<int,int,int> parameter;
	int row = 0;
	...
	map<tuple<int,int,int>,bool>::iterator it;
	...
	for (int cycle = 0; cycle < 6; cycle++) {
		...
		while( it != space.end() ) {
			...
			for (int x = -1; x <= 1; x++) {
				for (int y = -1; y <= 1; y++) {
					for (int z = -1; z <= 1; z++) {
						...
						parameter = {get<0>(it->first)+x, get<1>(it->first)+y, get<2>(it->first)+z};
			...
			if (count_on == 2 || count_on == 3) {
				next_matrix[{get<0>(it->first), get<1>(it->first), get<2>(it->first)}] = true;
				final_count ++;
			}
			++it;
		}
